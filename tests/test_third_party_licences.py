...
@pytest.mark.parametrize("dependency", ["PyTorch", "timesfm", "huggingface-hub", "pandas", "NumPy", "scikit-learn", "SciPy", "CCXT", "yfinance", "openai", "CustomTkinter", "reportlab"])
...
def test_the_document_names_no_dependency_that_was_dropped():
    """vectorbt was removed for its licence. A document that still
    lists it as a dependency would be describing a product that no longer
    exists.
    """
    for line in DOCUMENT.splitlines():
        if line.startswith("|") and "vectorbt" in line:
