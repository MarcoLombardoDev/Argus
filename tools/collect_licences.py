...
to accompany the object code, and Argus's own AGPL requires the same.
...
Argus is built with ``--onefile``, so unlike Orion there is no bundle
...
def collect(repo: str, staging: str) -> str:
    ...
    index = [
        ...
        "## Argus",
        "",
        "- `Argus-LICENSE.txt` — GNU Affero General Public License v3.0",
        "",
    ]

    shutil.copyfile(
        os.path.join(repo, "LICENSE"), os.path.join(staging, "Argus-LICENSE.txt")
    )
    ...
    index += [
        ...
        "`Argus-LICENSE.txt` and the sources in the repository cover",
        "Argus's own code.",
        "",
    ]
