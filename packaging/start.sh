#!/bin/sh
# Start Argus, after checking that the executable is the one this archive was
# built with.
#
...
if [ "${ARGUS_SKIP_VERIFY:-}" = "1" ]; then
    echo "$APP: checksum verification skipped (ARGUS_SKIP_VERIFY=1)" >&2
elif [ ! -f "$sums" ]; then
