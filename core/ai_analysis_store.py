...
def _results_to_df(results: list[dict]) -> pd.DataFrame:
    ...
    rows = []
    for r in results:
        tfm_sig = str(r.get("timefm_signal_1d", "N/A")).upper().strip()
        adv_sig = str(r.get("signal_1d", "N/A")).upper().strip()
        fng_value = r.get("market_context", {}).get("fng_value", 50)
        
        pm = PortfolioManager(load_settings())
        ...
        pm_conf = float(r.get("btc_pred_confidence") or 50.0)
        ai_conf = float(r.get("confidence") or 50.0)
        tfm_conf = float(r.get("tfm_confidence") or 50.0)
        ...
        # Retrieve leverage for ROI
        app_settings = load_settings()
        leverage_cfg = int(app_settings.get("portfolio_manager", {}).get("maxLeverage", 1))
        ...
        if curr_p and float(curr_p) > 0:
            ...
            if sl_val is not None:
                try:
                    if adv_sig == "SELL": sl_change = (float(curr_p) - float(sl_val)) / float(curr_p)
            ...
            if tp_val is not None:
                try:
                    if adv_sig == "SELL": tp_change = (float(curr_p) - float(tp_val)) / float(curr_p)
...
def export_session_pdf(session_id: str, output_path: str) -> bool:
    ...
    try:
        ...
        signal_colors = {"BUY": colors.Color(0, 0.6, 0.2), "SELL": colors.red, "HOLD": colors.orange, "N/A": colors.gray}
        ...
        def calc_pct(val, base, signal, lev):
            if val is None or base is None: return "N/A"
            try:
                b = float(base)
                v = float(val)
                if b <= 0: return f"{v:.4f}"
                if signal.upper() == "SELL":
                    change = (b - v) / b
                else:
                    change = (v - b) / b
                roi = change * lev * 100
                return f"{v:.4f} ({roi:+.2f}%)"
            except (ValueError, TypeError):
                return "N/A"
        ...
        app_settings = load_settings()
        leverage = int(app_settings.get("portfolio_manager", {}).get("maxLeverage", 1))
        ...
        for r in results:
            ...
            pm = PortfolioManager(app_settings)
            tfm_pct = float(r.get("change_pct_1d") or 0.0)
            pm_pct = float(r.get("btc_expected_move") or 0.0)
            ai_pct = float(r.get("ai_change_pct_1d") or tfm_pct)
            pm_conf = float(r.get("btc_pred_confidence") or 50.0)
            ...
            if r.get("stop_loss") is not None and curr_p and float(curr_p) > 0:
                try:
                    sl_dist = abs(float(curr_p) - float(r.get("stop_loss"))) / float(curr_p)
                    if sl_dist > 0:
                        safe_lev = int(0.80 / sl_dist)
                        leverage = min(leverage, max(1, safe_lev))
                except (ValueError, TypeError):
            ...
            row = [
                Paragraph(r.get("name", "")[:20], cell_style),
                Paragraph(r.get("symbol", ""), cell_style),
                ...
                Paragraph(r.get("signal_1d", "N/A"), cell_style),
                Paragraph(r.get("timefm_signal_1d", "N/A"), cell_style),
                Paragraph(calc_pct(r.get("stop_loss"), curr_p, adv_sig, leverage), cell_style),
                Paragraph(calc_pct(r.get("take_profit"), curr_p, adv_sig, leverage), cell_style),
                Paragraph(sizing_str, cell_style),
                Paragraph((r.get("rationale", "")[:80] + "...") if len(r.get("rationale","")) > 80 else r.get("rationale", ""), cell_style),
            ]
